# Mục lục tài liệu dự án (RTC Vision Management System Docs)

## 📌 Kiến trúc & Kỹ thuật cốt lõi
- [Tổng quan kiến trúc hệ thống](architecture/system_overview.md)
- [Cơ chế dung hợp dữ liệu Vision & FMS Homography](architecture/fusion_pipeline.md)
- [Kiến trúc định danh đa Camera ReID](architecture/reid_global_tracking.md)
- [Bộ xử lý quy tắc an ninh & ROI](architecture/security_rules_engine.md)

## 📖 Hướng dẫn sử dụng
- [Active Learning: sửa nhãn, dataset, retrain và kiểm định](guides/active_learning.md)
- [Workflow Editor và triển khai pipeline](guides/workflow_editor.md)
- [Upload model TensorRT và Calibration](guides/model_upload_and_calibration.md)
- [Đăng ký đa góc nhìn, Triplet metric và Calibration](identity-calibration.md)
- [Đăng ký nhãn vật thể với SAM 2](guides/sam2_target_registration.md)
- [Bản đồ 3D Digital Twin FMS](guides/fms_3d_digital_twin.md)
- [Tối ưu hóa hiệu năng GPU](guides/gpu_optimization.md)
- [Nhận diện 17 điểm dáng người & Té ngã](guides/pose_estimation_and_safety.md)

## 🔌 API & Triển khai
- [Danh mục REST API & WebSocket](api/endpoints_reference.md)
- [Triển khai môi trường Docker](deployment/docker_setup.md)
- [Bảng biến môi trường cấu hình](deployment/environment_variables.md)
- [Khắc phục sự cố Camera & RTSP](troubleshooting/camera_and_rtsp.md)
- [Khắc phục sự cố MQTT FMS VDA 5050](troubleshooting/fms_mqtt_sync.md)
- [Quy chuẩn đóng góp mã nguồn](CONTRIBUTING.md)
